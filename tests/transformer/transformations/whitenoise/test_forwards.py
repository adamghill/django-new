...
def test_whitenoise_adds_package_to_pyproject(fake_fs, temp_path):
...
""")
    ...
    content = (temp_path / "pyproject.toml").read_text()
    assert "whitenoise==6.6.0" in content
    assert "django>=4.2" in content


def test_whitenoise_adds_runserver_nostatic_to_installed_apps(fake_fs, temp_path):
...
""")
    ...
    content = (temp_path / "settings.py").read_text()
...
def test_whitenoise_adds_middleware_after_security(fake_fs, temp_path):
...
def test_whitenoise_configures_storages(fake_fs, temp_path):
...
def test_whitenoise_raises_error_if_already_in_dependencies(fake_fs, temp_path):
...
def test_whitenoise_complete_transformation(fake_fs, temp_path):
...
def test_whitenoise_rollback_after_error(fake_fs, temp_path):
